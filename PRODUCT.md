...
- `assets/i/retreats-holding/` (added 2026-08-19): 27 real, user-owned photos from the Isle of Mull, rehoused here before the `retreats/` folder was deleted (its content was AI-drafted exploration text — see below — and had no unique product content worth keeping; the images were the only thing worth preserving). Full quality/size unreviewed at scale — need resizing before use, per the same pattern as the Fionnphort master/hero split above. A 5-photo sample confirmed real, varied subject matter: coastal landscape shots, a retreat basecamp/campsite scene, an alternate angle of the boatbuilder's workshop already live on the home page, a black-and-white macro of old mill/loom machinery (`IMG_8182.jpeg`, confirmed by the user on 2026-08-19 to be Ardalanish Woollen Mill), and a candid group photo (needs rotation). Not yet vetted individually beyond that sample — confirm with the user before treating any specific one as ready to ship.
- The deleted `retreats/` folder's `45.md`/`45-rev.md` (AI-drafted, not otherwise usable) contained one real detail worth keeping for future copy: a "Past activities include" list — Fingal's Cave and Iona, natural wool-dyeing, a beach hunt and Ulva settlement visit, crater swimming near Tobermory, barley malting at a local distillery, fine-dining fish prep, wildlife photography with a Nikon ambassador — plus a liability note that wild swimming/kayaking are led by qualified local guides, not Clarity Lab staff directly.
